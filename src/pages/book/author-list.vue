    ...
    methods:{
        ...
        handleDelete(id) {
            this.$confirm('确定要删除吗?', '提示', {
                ...
                this.deleteRequest('/ms-book-manager/api/bookAuthor/delete',{id:id}).then(resp => {
                    if (resp == "") {
                        this.getListData();
                    }
                })
        ...
        getListData(){
            this.getRequest('/ms-book-manager/api/bookAuthor/getList', {pageNum:this.currentPage,pageSize:this.limit}).then(resp => {
                if (resp && !resp.status) {
